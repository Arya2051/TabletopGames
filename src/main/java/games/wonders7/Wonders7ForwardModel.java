...
// Usually this means that ForwardModel is stateless, and this is a good principle to adopt, but as ever there will always be exceptions.
    ...
    public void _next(AbstractGameState state, AbstractAction action){
        ...
        // Prints players hands and the sizes
         if (wgs.getCurrentPlayer() ==0 && wgs.getTurnAction(0)==null){
             ...
             for (int i = 0; i < wgs.getNPlayers(); i++) {
                 System.out.println(wgs.getPlayerWonderBoard(i).toString() + " "+ i + " --> " + wgs.getPlayerResources(i) + " --> " + wgs.getPlayerHand(i) + " --> " + wgs.getPlayedCards(i));
             }
            System.out.println("");
         }
    ...
    protected List<AbstractAction> _computeAvailableActions(AbstractGameState gameState) {
        ...
        for (int i=0; i<wgs.getPlayerHand(wgs.getCurrentPlayer()).getSize(); i++){ // Goes through each card in hand
            if (wgs.getPlayerHand(wgs.getCurrentPlayer()).get(i).isPlayable(wgs)&&wgs.getPlayerHand(wgs.getCurrentPlayer()).get(i).isFree(wgs)){
                actions.add((new FreeCard(wgs.getPlayerHand(wgs.getCurrentPlayer()).get(i).cardName)));
            }
            else if (wgs.getPlayerHand(wgs.getCurrentPlayer()).get(i).isPlayable(wgs)&&(wgs.getPlayerHand(wgs.getCurrentPlayer()).get(i).constructionCost.size()!=0)){ // If player can afford card and
        ...
        switch (wgs.currentAge) {
            ...
            case 3:
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    // MilitaryStructures (Red)
                    wgs.AgeDeck.add(new Wonder7Card("Arsenal", Wonder7Card.Wonder7CardType.MilitaryStructures, createCardHash(new Wonders7Constants.resources[]{Wonders7Constants.resources.wood, Wonders7Constants.resources.ore, Wonders7Constants.resources.textile}, new int[]{3,1,1}), createCardHash(new Wonders7Constants.resources[]{Wonders7Constants.resources.shield}, new int[]{3})));
